...
const columnDefs = ref([
  ...
  { field: 'updatedAt', headerName: '更新日期', filter: 'agDateColumnFilter', width: 140, cellClass: 'text-muted-foreground text-xs' },
  { field: 'description', headerName: '描述', minWidth: 300, cellClass: 'text-muted-foreground text-xs' },
])

const rowData = ref([
  { id: 1, name: 'MacBook Pro 14"', category: '電子產品', price: 69900, status: '有庫存', updatedAt: '2024-01-01', description: 'Apple M3 Pro 晶片，14 吋顯示器，18GB 統一記憶體' },
  { id: 2, name: 'iPad Air 11"', category: '電子產品', price: 19900, status: '有庫存', updatedAt: '2024-01-05', description: 'M2 晶片，11 吋顯示器，Apple Pencil Pro 支援' },
  { id: 3, name: 'iPhone 15 Pro', category: '電子產品', price: 36900, status: '缺貨', updatedAt: '2024-01-10', description: '鈦金屬設計，A17 Pro 晶片，48MP 主相機' },
  { id: 4, name: 'AirPods Pro 2', category: '配件', price: 7490, status: '有庫存', updatedAt: '2024-01-15', description: '主動式降噪，H2 晶片，USB-C 充電盒' },
  { id: 5, name: 'Magic Mouse', category: '配件', price: 2290, status: '有庫存', updatedAt: '2024-01-20', description: '多點觸控表面，藍牙連線' },
  { id: 6, name: 'Studio Display', category: '顯示器', price: 45900, status: '低庫存', updatedAt: '2024-01-25', description: '27 吋 5K Retina 顯示器，12MP 超廣角相機' },
  { id: 7, name: 'Apple Watch Ultra 2', category: '配件', price: 27900, status: '有庫存', updatedAt: '2024-01-28', description: '航太等級鈦金屬，S9 SiP 晶片' },
  { id: 8, name: 'Mac Studio', category: '電子產品', price: 64900, status: '有庫存', updatedAt: '2024-02-01', description: 'M2 Max 晶片，精巧高效的桌上型電腦' },
])
...
<template>
  <div class="flex flex-col gap-6 w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between px-1">
      <div class="flex flex-col gap-1">
        <div class="flex items-center gap-2.5">
          <div class="p-2.5 bg-primary/10 rounded-xl text-primary shadow-sm border border-primary/20">
            <Grid :size="22" />
          </div>
          <div>
      ...
      <div class="flex items-center gap-3 mt-4 sm:mt-0">
        <Button variant="outline" size="sm" @click="handleCopy" class="h-10 px-4 rounded-lg shadow-sm hover:bg-accent transition-all">
    ...
    <div class="grid gap-6">
      <Card class="border-border/40 shadow-xl shadow-black/5 dark:shadow-none overflow-hidden rounded-2xl">
        <CardHeader class="pb-5 pt-6 bg-muted/20 border-b border-border/40">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20">
              ...
              <span class="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">Active</span>
            </div>
          </div>
        </CardHeader>
        <CardContent class="p-0">
          ...
          <div class="bg-muted/30 border-t border-border/40 p-6">
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <div class="group space-y-2 p-4 rounded-xl bg-background/50 border border-border/40 hover:border-primary/30 hover:shadow-md transition-all duration-300">
                ...
                <p class="text-xs text-muted-foreground leading-relaxed">
                  已修正深色模式下的底色 inheritance，確保在深色背景下表格內容呈現深 Slate 色調，符合整體 UI 質感。
                </p>
              </div>
              <div class="group space-y-2 p-4 rounded-xl bg-background/50 border border-border/40 hover:border-primary/30 hover:shadow-md transition-all duration-300">
                ...
                <p class="text-xs text-muted-foreground leading-relaxed">
                  實作 `onCellKeyDown` 邏輯，當選中單元格按下 Enter 鍵時立即啟動編輯模式，提升資料錄入效率。
                </p>
              </div>
              <div class="group space-y-2 p-4 rounded-xl bg-background/50 border border-border/40 hover:border-primary/30 hover:shadow-md transition-all duration-300">
                ...
                <p class="text-xs text-muted-foreground leading-relaxed">
                  透過 CSS 選取器美化選取框並在右下角加入主色方框（Fill Handle），模擬 Excel 視覺體驗。
                </p>
              </div>
            </div>
            
            <!-- Warning/Note about Enterprise features -->
            <div class="mt-6 flex items-start gap-4 p-4 rounded-xl bg-amber-500/5 border border-amber-500/20 shadow-inner">
              ...
              <div class="text-xs text-amber-800/80 dark:text-amber-300/70 leading-relaxed">
                <p class="font-bold mb-1 text-amber-900 dark:text-amber-200 uppercase tracking-tight text-[11px]">技術說明</p>
                <p>
                  我們使用了 **AG Grid Community** 版本透過自定義 CSS 與事件攔截來達成視覺與操作上的優化。Fill Handle 的實際「拖曳複製」邏輯在社群版中需手動實作區域計算，本展示已完成視覺整合與編輯流程優化。
                </p>
              </div>
            </div>
          </div>
